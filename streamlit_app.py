...
def compute_note_accrued_interest(principal, annual_rate_pct, start_date, end_date, interest_method, compounding_frequency):
    ...
    periods_map = {"annual": 1, "semi_annual": 2, "quarterly": 4, "monthly": 12, "daily": 365}
...
def build_conversion_rows(financing_df: pd.DataFrame, cap_table_for_conversion: pd.DataFrame, equity_round_name: str, equity_round_date, round_price: float, pre_round_fd_shares: float):
...
def add_starting_row():
    ...
    if holder and shares > 0:
        ...
        st.session_state.cap_table = pd.concat([st.session_state.cap_table, new_row], ignore_index=True)
...
def build_preferred_class_terms(cap_table: pd.DataFrame, round_history: pd.DataFrame, financing_details: pd.DataFrame) -> pd.DataFrame:
    ...
    if preferred.empty:
        return pd.DataFrame(columns=["class", "shares", "original_investment", "liq_pref_multiple", "participating", "participation_cap_multiple", "round_date", "round_order"])
    preferred_summary = preferred.groupby("class", dropna=False, as_index=False)["shares"].sum().rename(columns={"shares": "shares"})
    issued_equity = financing_details.copy()
    if not issued_equity.empty:
        issued_equity = issued_equity[(issued_equity["instrument_type"] == "Equity") & (issued_equity["status"] == "issued")].copy()
    if issued_equity.empty:
        ...
        investment_summary = issued_equity.groupby("round_name", as_index=False)["amount_invested"].sum().rename(columns={"round_name": "class", "amount_invested": "original_investment"})
    ...
    if not equity_rounds.empty:
        ...
        equity_rounds = equity_rounds[["round_name", "round_date", "liq_pref_multiple", "participating", "participation_cap_multiple", "round_order"]].rename(columns={"round_name": "class"})
    else:
        equity_rounds = pd.DataFrame(columns=["class", "round_date", "liq_pref_multiple", "participating", "participation_cap_multiple", "round_order"])
...
def allocate_capped_pro_rata(total_amount: float, participant_df: pd.DataFrame) -> dict:
    ...
    while remaining > 1e-9 and not working.empty:
        ...
        capped = active[active["cap_remaining"].notna() & (active["provisional"] > active["cap_remaining"] + 1e-9)].copy()
        if capped.empty:
...
def build_waterfall_for_exit(cap_table: pd.DataFrame, round_history: pd.DataFrame, financing_details: pd.DataFrame, exit_value: float):
    eligible = get_exit_eligible_cap_table(cap_table)
    if eligible.empty:
    ...
    if not class_summary_df.empty:
        ...
        if not pref_classes.empty:
            pref_classes["sort_date"] = pref_classes["round_date_parsed"].apply(lambda x: x.toordinal() if x is not None else -1)
            pref_classes = pref_classes.sort_values(by=["sort_date", "round_order"], ascending=[False, False])
        ...
        participants = []
        if common_shares > 0:
            participants.append({"participant_id": "COMMON_POOL", "shares": common_shares, "cap_remaining": None})
        for _, row in class_summary_df.iterrows():
            if bool(row["participating"]):
                ...
                participants.append({"participant_id": f"CLASS::{row['class']}", "shares": float(row["shares"]), "cap_remaining": cap_remaining})
            elif bool(row["convert_to_common"]):
                participants.append({"participant_id": f"CLASS::{row['class']}", "shares": float(row["shares"]), "cap_remaining": None})
        ...
        class_summary_df["residual_allocated"] = class_summary_df["class"].apply(lambda c: residual_allocations.get(f"CLASS::{c}", 0.0))
        class_summary_df["total_payout"] = class_summary_df["pref_allocated"] + class_summary_df["residual_allocated"]
    else:
    ...
    if common_shares > 0:
        for _, row in common_rows.iterrows():
            ...
            holder_rows.append({"holder": row["holder"], "security_type": row["security_type"], "class": row["class"], "shares": float(row["shares"]), "issue_date": row.get("issue_date"), "exit_proceeds": payout})
    ...
    if not preferred_rows.empty and not class_summary_df.empty:
        ...
        for _, row in preferred_rows.iterrows():
            ...
            holder_rows.append({"holder": row["holder"], "security_type": row["security_type"], "class": class_name, "shares": float(row["shares"]), "issue_date": row.get("issue_date"), "exit_proceeds": payout})
    ...
    if class_summary_df.empty:
        class_summary_df = pd.DataFrame(columns=["class", "shares", "original_investment", "liq_pref_multiple", "participating", "participation_cap_multiple", "as_converted_value", "pref_claim", "convert_to_common", "pref_allocated", "residual_allocated", "total_payout"])
    else:
        class_summary_df = class_summary_df[["class", "shares", "original_investment", "liq_pref_multiple", "participating", "participation_cap_multiple", "as_converted_value", "pref_claim", "convert_to_common", "pref_allocated", "residual_allocated", "total_payout"]].copy()
    return holder_df, class_summary_df


def build_exit_sensitivity_table(cap_table: pd.DataFrame, round_history: pd.DataFrame, financing_details: pd.DataFrame, exit_values: list[float]) -> pd.DataFrame:
    eligible = get_exit_eligible_cap_table(cap_table)
    ...
    for exit_value in exit_values:
        holder_df, _ = build_waterfall_for_exit(cap_table=cap_table, round_history=round_history, financing_details=financing_details, exit_value=float(exit_value))
        col_name = f"${exit_value:,.0f}"
        payout_map = {}
        if not holder_df.empty:
            payout_map = holder_df.set_index(["holder", "security_type", "class", "issue_date"])["exit_proceeds"].to_dict()
        result[col_name] = result.apply(lambda r: payout_map.get((r["holder"], r["security_type"], r["class"], r["issue_date"]), 0.0), axis=1)
    return result


def validate_round_inputs(round_type: str, round_name: str, investors: list, pre_money_valuation, valuation_cap, discount_pct):
    errors = []
    clean_round_name = round_name.strip() or round_type
    valid_investors = [inv for inv in investors if inv["investor_name"].strip() and float(inv["amount"] or 0.0) > 0]
...
def format_money_columns(df: pd.DataFrame, columns: list[str], decimals: int = 2) -> pd.DataFrame:
    ...
    for col in columns:
        if col in out.columns:
            out[col] = out[col].apply(lambda x: f"${x:,.{decimals}f}" if pd.notnull(x) and x != "" else "")
    return out


def format_share_columns(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    out = df.copy()
    for col in columns:
        if col in out.columns:
            out[col] = out[col].apply(lambda x: f"{x:,.0f}" if pd.notnull(x) and x != "" else "")
    return out
...
with tab1:
    ...
    with st.form("add_starting_common_holder_form"):
        ...
        col2.text_input("Common shares", value=st.session_state.get("new_shares", "0"), key="new_shares", help="You can type values with commas, like 1,250,000.")
    ...
    existing_common_holders = sorted(st.session_state.cap_table.loc[st.session_state.cap_table["security_type"] == "Common", "holder"].dropna().astype(str).unique().tolist())
    c1, c2 = st.columns(2)
    c1.selectbox("Change type", ["Increase existing holder", "Add new holder"], key="common_change_type")
    if st.session_state.get("common_change_type") == "Increase existing holder":
        c2.selectbox("Existing common holder", options=[""] + existing_common_holders, key="common_change_holder")
    else:
        c2.text_input("New holder name", key="common_change_new_holder")
    c3, c4 = st.columns(2)
    c3.text_input("Additional common shares", value=st.session_state.get("common_change_shares", "0"), key="common_change_shares", help="You can type values with commas.")
    ...
    option_holders = sorted(st.session_state.cap_table.loc[st.session_state.cap_table["security_type"] == "Common", "holder"].dropna().astype(str).unique().tolist())
    o1, o2, o3 = st.columns(3)
    o1.selectbox("Common holder receiving options", options=[""] + option_holders, key="option_grant_holder")
    o2.text_input("Options granted", value=st.session_state.get("option_grant_shares", "0"), key="option_grant_shares", help="You can type values with commas.")
    ...
    if st.button("Issue options"):
        ...
        current_pool = 0.0
        if pool_mask.any():
            current_pool = pd.to_numeric(st.session_state.cap_table.loc[pool_mask, "shares"], errors="coerce").fillna(0.0).sum()
        if not option_holder:
            ...
            if remaining_pool > 0:
                pool_row = pd.DataFrame([{"holder": "Option Pool Reserve", "security_type": "Option Pool", "class": "Option Pool", "shares": remaining_pool, "issue_date": date_to_str(option_issue_date)}])
                updated_cap = pd.concat([updated_cap, pool_row], ignore_index=True)
            option_row = pd.DataFrame([{"holder": option_holder, "security_type": "Option", "class": "Employee Option", "shares": option_shares, "issue_date": date_to_str(option_issue_date)}])
            updated_cap = pd.concat([updated_cap, option_row], ignore_index=True)
            st.session_state.cap_table = updated_cap
            st.session_state["_pending_widget_state_updates"] = {**st.session_state.get("_pending_widget_state_updates", {}), "option_pool_shares": format_number_for_input(remaining_pool), "option_pool_issue_date": option_issue_date, "option_grant_shares": "0"}
            st.rerun()
    ...
    p1, p2 = st.columns(2)
    p1.text_input("Option pool reserve shares", value=st.session_state.get("option_pool_shares", "0"), key="option_pool_shares", help="You can type values with commas.")
...
with tab2:
    ...
    if st.session_state.cap_table.empty:
        ...
        if round_type == "Equity":
            pre_money_valuation = parse_numeric(text_numeric_input("Pre-money valuation ($)", key="equity_pre_money_valuation", value=st.session_state.get("equity_pre_money_valuation", "0"), help_text="You can type values with commas, like 25,000,000."))
            st.markdown("### Preferred stock terms")
            liq_pref_multiple = parse_numeric(text_numeric_input("Liquidation preference multiple (x)", key="liq_pref_multiple", value=st.session_state.get("liq_pref_multiple", "1")))
            ...
            if participating:
                ...
                if has_participation_cap:
                    participation_cap_multiple = parse_numeric(text_numeric_input("Participation cap multiple (x)", key="participation_cap_multiple", value=st.session_state.get("participation_cap_multiple", "3")))
        elif round_type == "SAFE":
            valuation_cap = parse_numeric(text_numeric_input("Valuation cap ($)", key="safe_valuation_cap", value=st.session_state.get("safe_valuation_cap", "0")))
            discount_pct = parse_numeric(text_numeric_input("Discount (%)", key="safe_discount_pct", value=st.session_state.get("safe_discount_pct", "0")))
            issue_date = round_date
        else:
            valuation_cap = parse_numeric(text_numeric_input("Valuation cap ($)", key="note_valuation_cap", value=st.session_state.get("note_valuation_cap", "0")))
            discount_pct = parse_numeric(text_numeric_input("Discount (%)", key="note_discount_pct", value=st.session_state.get("note_discount_pct", "0")))
            interest_rate_pct = parse_numeric(text_numeric_input("Interest rate (%)", key="note_interest_rate_pct", value=st.session_state.get("note_interest_rate_pct", "0")))
            issue_date = round_date
        ...
        if round_type == "Equity":
            ...
            e2.markdown("**Conversion rule:** Convert outstanding SAFEs and notes in this priced round")
            option_pool_timing = e3.selectbox("Option pool timing", ["pre_money", "post_money"], key="equity_option_pool_timing")
            pre_money_fd_shares_override = parse_numeric(st.text_input("Pre-money fully diluted shares override", value=st.session_state.get("equity_pre_money_fd_shares_override", "0"), key="equity_pre_money_fd_shares_override", help="Optional. Leave 0 to use current cap table fully diluted shares."))
        elif round_type == "SAFE":
            s1, s2, s3 = st.columns(3)
            safe_type = "post_money_safe"
            s1.markdown("**SAFE type:** Post-money SAFE")
            capitalization_basis = s2.selectbox("Capitalization basis", ["company_cap_table_fd", "exclude_option_pool_reserve", "outstanding_only"], key="safe_capitalization_basis")
            shadow_preferred = s3.checkbox("Shadow preferred", key="safe_shadow_preferred", value=False)
        else:
            n1, n2, n3 = st.columns(3)
            capitalization_basis = n1.selectbox("Capitalization basis", ["company_cap_table_fd", "exclude_option_pool_reserve", "outstanding_only"], key="note_capitalization_basis")
            interest_method = n2.selectbox("Interest method", ["simple", "compound"], key="note_interest_method")
            interest_compounding_frequency = n3.selectbox("Compounding frequency", ["annual", "semi_annual", "quarterly", "monthly", "daily"], key="note_interest_compounding_frequency")
            converts_accrued_interest = st.checkbox("Convert accrued interest", key="note_converts_accrued_interest", value=True)
        ...
        for i in range(int(investor_count)):
            ...
            amount = parse_numeric(c2.text_input(f"Amount invested {i+1} ($)", value=st.session_state.get(f"inv_amt_{i}", "0"), key=f"inv_amt_{i}", help="You can type values with commas."))
        ...
        if round_type == "Equity":
            ...
            if not outstanding.empty:
                ...
                outstanding_cols = ["instrument_type", "investor_name", "amount_invested", "principal_invested", "valuation_cap", "discount_pct", "interest_rate_pct", "capitalization_basis", "issue_date"]
                outstanding_show = outstanding[[c for c in outstanding_cols if c in outstanding.columns]].copy()
                outstanding_show = format_money_columns(outstanding_show, ["amount_invested", "principal_invested", "valuation_cap"])
                st.dataframe(outstanding_show, use_container_width=True)
        if st.button("Apply funding round"):
            clean_round_name, valid_investors, errors = validate_round_inputs(round_type=round_type, round_name=round_name, investors=investors, pre_money_valuation=pre_money_valuation, valuation_cap=valuation_cap, discount_pct=discount_pct)
            ...
            if errors:
                ...
                st.session_state.round_history = pd.concat([st.session_state.round_history, round_row], ignore_index=True)
                if round_type == "Equity":
                    ...
                    for inv in valid_investors:
                        ...
                        preferred_rows.append({"holder": inv["investor_name"], "security_type": "Preferred", "class": clean_round_name, "shares": shares_issued, "issue_date": date_to_str(round_date)})
                    convertible_rows, financing_updated, conversion_summary = build_conversion_rows(st.session_state.financing_details, updated_cap_table, clean_round_name, round_date, round_price, pre_round_shares)
                    if preferred_rows:
                        updated_cap_table = pd.concat([updated_cap_table, pd.DataFrame(preferred_rows)], ignore_index=True)
                    if convertible_rows:
                        updated_cap_table = pd.concat([updated_cap_table, pd.DataFrame(convertible_rows)], ignore_index=True)
                    ...
                    if equity_financing_rows:
                        st.session_state.financing_details = pd.concat([st.session_state.financing_details, pd.DataFrame(equity_financing_rows)], ignore_index=True)
                    ...
                    if not conversion_summary.empty:
                        ...
                        for col in ["conversion_amount", "accrued_interest", "cap_price", "discount_price", "round_price", "conversion_price"]:
                            if col in show_conv.columns:
                                show_conv[col] = show_conv[col].apply(lambda x: f"${x:,.4f}" if pd.notnull(x) else "")
                    ...
                    if safe_rows:
                        st.session_state.financing_details = pd.concat([st.session_state.financing_details, pd.DataFrame(safe_rows)], ignore_index=True)
                    st.session_state.cap_table = updated_cap_table
                    st.success("SAFE round stored. It will convert automatically in a later priced equity round.")
                else:
                    note_rows = []
                    for inv in valid_investors:
                        note_rows.append({
                    ...
                    if note_rows:
                        st.session_state.financing_details = pd.concat([st.session_state.financing_details, pd.DataFrame(note_rows)], ignore_index=True)
                    st.session_state.cap_table = updated_cap_table
                    st.success("Convertible note round stored. It will convert automatically in a later priced equity round.")
...
with tab3:
    ...
    if st.session_state.round_history.empty:
        ...
        history_show = format_money_columns(history_show, ["pre_money_valuation", "amount_raised", "valuation_cap", "price_per_share_override"])
    ...
    if st.session_state.financing_details.empty:
        ...
        details_show = format_money_columns(details_show, ["amount_invested", "principal_invested", "valuation_cap", "conversion_amount", "cap_price", "discount_price", "conversion_price", "round_price_at_conversion"])
...
with tab4:
    ...
    if current.empty:
        ...
        low_exit = parse_numeric(col1.text_input("Low exit value ($)", value=st.session_state.get("low_exit_value", "10,000,000"), key="low_exit_value", help="You can type values with commas."))
        high_exit = parse_numeric(col2.text_input("High exit value ($)", value=st.session_state.get("high_exit_value", "200,000,000"), key="high_exit_value", help="You can type values with commas."))
        preview_exit = parse_numeric(st.text_input("Single exit value preview ($)", value=st.session_state.get("preview_exit_value", "50,000,000"), key="preview_exit_value", help="You can type values with commas."))
        if st.button("Run exit sensitivity"):
            ...
            if not exit_values:
                ...
                sensitivity_df = build_exit_sensitivity_table(st.session_state.cap_table, st.session_state.round_history, st.session_state.financing_details, exit_values)
                display_df = sensitivity_df.copy()
                display_df = format_share_columns(display_df, ["shares"])
                if "ownership_pct_ex_option_pool" in display_df.columns:
                    display_df["ownership_pct_ex_option_pool"] = display_df["ownership_pct_ex_option_pool"].map(lambda x: f"{x:.2%}")
                ...
                st.dataframe(display_df, use_container_width=True)
                preview_holder_df, preview_class_df = build_waterfall_for_exit(st.session_state.cap_table, st.session_state.round_history, st.session_state.financing_details, preview_exit)
                st.write(f"### Waterfall preview at ${preview_exit:,.0f}")
                if not preview_class_df.empty:
                    preview_class_show = preview_class_df.copy()
                    preview_class_show = format_money_columns(preview_class_show, ["original_investment", "as_converted_value", "pref_claim", "pref_allocated", "residual_allocated", "total_payout"])
                ...
                st.download_button("Download exit sensitivity CSV", data=to_csv_bytes(sensitivity_df), file_name="exit_sensitivity.csv", mime="text/csv")

with tab5:
    st.subheader("Downloads")
    current = recalc_ownership(st.session_state.cap_table)
    st.download_button("Download cap table CSV", data=to_csv_bytes(current), file_name="cap_table_current.csv", mime="text/csv")
    ...
    uploaded_financing = st.file_uploader("Upload financing details CSV", type=["csv"], key="uploaded_financing_details")
    if st.button("Load uploaded CSVs"):
        try:
            if uploaded_cap is not None:
                ...
                if pool_mask.any():
                    pending_updates["option_pool_shares"] = format_number_for_input(pd.to_numeric(st.session_state.cap_table.loc[pool_mask, "shares"], errors="coerce").fillna(0.0).sum())
